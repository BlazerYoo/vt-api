...
class VirusTotalResult:
    ...
    def url(self):
        return f"https://www.virustotal.com/gui/url/{self.id}/detection"


class VirusTotalDetection:
    UNDETECTED = 'Undetected'
    UNABLE_TO_PROCESS = [
        'Unable to process url type',
        'Timeout',
        'Confirmed timeout',
        ''
    ]
...
def webdriver(headless=False):
    ...
    driver = Chrome(executable_path='.'+os.sep+'chromedriver_win32'+os.sep+'chromedriver.exe',options=options)
    yield driver
    driver.quit()
...
def get_detection_details(driver):
    ...
    detections_list = wait_for_elem(driver, ['url-view', 'vt-ui-detections-list', '#detections'])
...
def get_analysis_results(driver):
    ...
    url_id_element = wait_for_elem(driver, ['url-view', 'vt-ui-url-card', '.url-id'])
    WebDriverWait(driver, WAIT_TIME).until(lambda x: url_id_element.text != '')
    result.id = url_id_element.text
    total_element = wait_for_elem(driver, ['url-view', 'vt-ui-main-generic-report', 'vt-ui-detections-widget', '.engines .circle .total'])
    WebDriverWait(driver, WAIT_TIME).until(lambda x: total_element.text != '')
    result.total_results = extract_int(total_element.text)
    malicious_element = wait_for_elem(driver, ['url-view', 'vt-ui-main-generic-report', 'vt-ui-detections-widget', '.engines .circle .positives'])
...
def set_url(driver, url):
    input = wait_for_elem(driver, '#input')
    sleep(1)
    input.send_keys(url)
    sleep(1)
...
def is_analysis_page(driver):
    url = "https://www.virustotal.com/gui/url-analysis/"
    return url in driver.current_url
...
def analyze(url, headless=False, progress_callback = lambda percent_complete: None):
...
def analyze(url):
    ...
    scan_url = 'https://www.virustotal.com/gui/url/{}/detection'.format(hash)
    options = Options()
    options.headless = True
    #options.binary_location = 'C:\\Users\\BY\\Documents\\GitHub\\vt-api\\vtpy\\Win_x64_906559_chrome-win\\chrome-win\\chrome.exe'
    chromedriver_binary_location = '.'+os.sep+'chromedriver_win32'+os.sep+'chromedriver.exe'
    #'.\\Win_x64_906559_chromedriver_win32\\chromedriver_win32\\chromedriver.exe'
    with Chrome(executable_path=chromedriver_binary_location, options=options) as driver:
        driver.get(scan_url)
